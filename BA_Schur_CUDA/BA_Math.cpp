...
void MatMulEa(dtype Aij[2][15], int i)
{
	...
	//Comment the error calculation and use Ep vector
...
void invCalculation(dtype* V)
{
	...
	V[0] = ((e*i) - (h*f)) / det; V[1] = ((h*c) - (b*i)) / det; V[2] = ((b*f) - (e*c)) / det;
	V[3] = ((f*g) - (d*i)) / det; V[4] = ((a*i) - (g*c)) / det; V[5] = ((d*c) - (a*f)) / det;
	V[6] = ((d*h) - (e*g)) / det; V[7] = ((b*g) - (a*h)) / det; V[8] = ((a*e) - (b*d)) / det;
}
/*
...
{
	dtype tmp;
	//cout << "Cholesky decomposition entered" << endl;
	for (int i = 0; i < n; i++){
		for (int j = 0; j <= i; j++){
			tmp = 0;
			for (int k = 0; k < j; k++){
	...
	//cout << "Cholesky decomposition exit" << endl;
}
