        ...
        do je = 1, ney
          do j = 1, nx-1
            ...
            do ie = 1, nex
              do i = 1, nx-1
                iu = (ie-1)*(nx-1) + i
                fglob(iu,ju,ku) = global_vars_phys_thp(i,j,k,ie,je,ke) 
              end do
            end do
...
!! Global data on 3D domain 
    ...
    do ke = 1, nez
      do je = 1, ney
        do ie = 1, nex
          dat3d(:,:,:,ie,je,ke) = global_vars_phys_thp(:,:,:,ie,je,ke) 
        end do
      end do
    end do
        ...
        do je = 1, ney
          do j = 1, nx-1
            ...
            do ie = 1, nex
              do i = 1, nx-1
                iu = (ie-1)*(nx-1) + i
                uf(iu,ju,ku) = global_vars_phys_u(i,j,k,ie,je,ke)
               !vf(iu,ju,ku) = vars(ie,je,ke)%spd(i,j,k)
                vf(iu,ju,ku) = global_vars_phys_v(i,j,k,ie,je,ke)
                wf(iu,ju,ku) = global_vars_phys_w(i,j,k,ie,je,ke) 
              end do
            end do
    ...
    do ke = 1, nez
      do k = 1, nx-1
        ...
        do ie = 1, nex
          do i = 1, nx-1
            iu = (ie-1)*(nx-1) + i
            fcut(iu,ku) = global_vars_phys_psi(i,j,k,ie,je,ke)
          end do
        end do
...
!    type(element_t), intent(in), dimension(nex,ney,nez) ::  vars, vars_init 
   ...
   if (vec_id == 5) then
    do ke = 1, nez
      do je = 1, ney
        do ie = 1, nex
          ref(:,:,:,ie,je,ke) = global_vars_init_rho(:,:,:,ie,je,ke) * global_vars_init_the(:,:,:,ie,je,ke) 
          err(:,:,:,ie,je,ke) = global_vars_rho(:,:,:,ie,je,ke) * global_vars_the(:,:,:,ie,je,ke) - ref(:,:,:,ie,je,ke)
        end do
      end do
    end do
   ...
   if (vec_id == 2) then
    do ke = 1, nez
      do je = 1, ney
        do ie = 1, nex
          ref(:,:,:,ie,je,ke) = global_vars_init_u(:,:,:,ie,je,ke) * global_vars_init_rho(:,:,:,ie,je,ke)
          err(:,:,:,ie,je,ke) = global_vars_u(:,:,:,ie,je,ke)* global_vars_rho(:,:,:,ie,je,ke) - ref(:,:,:,ie,je,ke)
        end do
      end do
    end do
