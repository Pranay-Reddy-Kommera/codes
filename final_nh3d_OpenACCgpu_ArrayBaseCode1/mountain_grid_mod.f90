...
call varsToVarsPhys()
    ...
    do ke = 1, nlz
      do je = 1, nly
        do ie = 1, nlx
          do k = 1, nx
            vars_phys_psi(:,:,k,ie,je,ke) = vars_psi(:,:,k,ie,je,ke) / metric_sg(:,:,ie,je,ke)
            vars_phys_rho(:,:,k,ie,je,ke) = vars_rho(:,:,k,ie,je,ke) / metric_sg(:,:,ie,je,ke)
          end do
        end do
      end do
    end do
          ...
          do k = 1, nx
            do j = 1, nx
              do i = 1, nx
                ...
                zeta = grid_z(i,j,k,ie,je,ke)

                call Compute_metric_at_coords(x, y, zeta, &
                     tmp_mtn, tmp_ght, tmp_sg, tmp_sg13, tmp_sg23)

                ! mountain and coordinate heights
                metric_mtn(i,j,ie,je,ke) = tmp_mtn
                metric_ght(i,j,k,ie,je,ke) = tmp_ght

                ! metric terms
                metric_sg(i,j,ie,je,ke) = tmp_sg
                metric_sg13(i,j,k,ie,je,ke) = tmp_sg13
                metric_sg23(i,j,k,ie,je,ke) = tmp_sg23
              end do
            end do
          end do
        end do
...
!    Type(element_t), intent(in) :: vars
    ...
    do k = 1, nx
      do j = 1, nx
        do i = 1, nx
          sg13 = metric_sg13(i,j,k,ie,je,ke)
          sg23 = metric_sg23(i,j,k,ie,je,ke)
          sg = metric_sg(i,j,ie,je,ke)

          ! Based on G-S transform
          wt(i,j,k) = (vars_w(i,j,k,ie,je,ke) + sg13 * vars_u(i,j,k,ie,je,ke) + sg23 * vars_v(i,j,k,ie,je,ke))/sg
        end do
      end do
    end do
